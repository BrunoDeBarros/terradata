<?php
/**
 * Terra Duo Collection
 *
 * Allows the complete management of a data collection.
 *
 ...
 */
class Terra_Table_Collection implements Terra_Table_Interface {
    ...
    public function setTableData($TableArray) {
        if (is_array($TableArray)) {
            ...
            if (file_exists($TableArray)) {
                ...
                throw new Terra_Table_Exception("Could not include $TableArray for use in Terra_Table_Collection->setTableData().");
            }
        }
    }
    ...
    public function  __call($name,  $arguments) {
        if ((!stristr($name, 'get') OR !stristr($name, 'by')) and (!stristr($name, 'set') OR !stristr($name, 'by'))) {
            throw new Terra_Table_Exception("Invalid method call: \$Terra_Table_Collection->$name() is an invalid method name.");
            return false;
        }
    ...
    public function query($sql) {
        if (!is_resource($this->DatabaseConnection)) {
            throw new Terra_Table_Exception('To execute a query, you must set a valid database connection, using $Terra_Table_Collection->setDatabaseConnection().');
        }
        $start = microtime();
