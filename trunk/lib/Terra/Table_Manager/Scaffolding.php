...
class TD_Table_Scaffolding {
    ...
    /**
     * The TD_Table Instance.
     * @var TD_Table_Interface
     */
    public $Table;
    /**
    ...
    public function setTableData($TableArray, $TD_Table = null) {
        if (is_array($TableArray)) {
            ...
            if (!isset($TableArray['BaseURL'])) {
                throw new TD_Table_Exception("You didn't set a BaseURL in your table definition!");
            }
            if (!isset($TableArray['View'])) {
                throw new TD_Table_Exception("You didn't set a View URL in your table definition!");
            }
            if (!isset($TableArray['Edit'])) {
                throw new TD_Table_Exception("You didn't set an Edit URL in your table definition!");
            }
            if (!isset($TableArray['Delete'])) {
                throw new TD_Table_Exception("You didn't set a Delete URL in your table definition!");
            }
            if (!isset($TableArray['Manage'])) {
                throw new TD_Table_Exception("You didn't set a Manage URL in your table definition!");
            }
            if (!isset($TableArray['Create'])) {
                throw new TD_Table_Exception("You didn't set a Create URL in your table definition!");
            }
            if (!isset($TableArray['Restore'])) {
                throw new TD_Table_Exception("You didn't set a Restore URL in your table definition!");
            }
            ...
            if (file_exists($TableArray)) {
                ...
                throw new TD_Table_Exception("Could not include $TableArray for use in TD_Table_Scaffolding->setTableData().");
            }
        }

        if ($TD_Table === null) {
            if (!isset($TableArray['TD_Table'])) {
                $this->Table = TD_Table_Manager::Factory('MySQL');
            } elseif (is_string($TableArray['TD_Table'])) {
                $this->Table = TD_Table_Manager::Factory($TableArray['TD_Table']);
            }
        } elseif($TD_Table instanceof TD_Table_Interface) {
            $this->Table = $TD_Table;
        } else {
            throw new TD_Table_Exception("A value for \$TD_Table was provided but this value does not implement TD_Table_Interface.");
            return false;
        }
    ...
    public function prepareURL($URL, $replace = array('{ID}' => 0)) {
        if (is_array($URL)) {
            if (!isset($URL[0]) or !isset($URL[1])) {
                throw new TD_Table_Exception("If you're trying to use prepareURL(\$args), you need to provide an array in the format: array(url, replace_array)");
                return;
            }
            $replace = $URL[1];
            $URL = $URL[0];
    ...
    public function EditController($ID = null) {
        if (isset ($_POST['SCAFFOLDING'])) {
            ...
            foreach ($Row[0] as $FieldName => $FieldValue) {
                ...
                TD_Events::trigger('TD_Table_Scaffolding_Edit_'.$FieldName, $Args);
            }
        ...
        foreach ($this->Table->getWhere($this->GlobalWhereClause, $args) as $Row) {
            ...
            foreach($this->TableFields as $FieldName => $FieldArray) {
                if ($FieldArray['DisplayOnManage']) {
                    ...
                    TD_Events::trigger('TD_Table_Scaffolding_Manage_'.$FieldName, $Args);
        ...
        if (!empty($ID) and $this->Table->count(array_merge($this->GlobalWhereClause, array('ID' => $ID))) > 0) {
            ...
            foreach($this->TableFields as $FieldName => $FieldArray) {
                if ($FieldArray['DisplayOnView']) {
                    ...
                    TD_Events::trigger('TD_Table_Scaffolding_View_'.$FieldName, $Args);
